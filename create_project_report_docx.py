...
def create_report_docx(output_path):
    ...
    toc_items = [
        ...
        ("", "4.2 DETAILED DESIGN & UML MODELS", "16"),
        ("", "    4.2.1 DATA FLOW DIAGRAM (DFD LEVEL 0 & LEVEL 1)", "16"),
    ...
    p_arch.add_run(
        "The Smart Health Risk Indicator architecture is organized into four decoupled, browser-native tiers:\n\n"
        "1. Multimodal Data Ingestion Layer: Captures standardized psychometric ratings (PHQ-9 depression scale, GAD-7 anxiety scale, PSQI sleep index), free-text patient journal narratives, and continuous lifestyle biomarkers (sleep hours, screen time, physical exercise frequency).\n\n"
        "2. Feature Preprocessing & NLP Pipeline: Performs min-max normalization, z-score standardization, lexical sentiment extraction, VADER polarity scoring, and emergency crisis keyword tripwire screening.\n\n"
        "3. Four-Model Stacked Ensemble Core:\n"
        "   • DepNet: BiLSTM with temporal self-attention specialized for depressive symptom severity.\n"
        "   • AnxNet: 1D-CNN with residual skip connections capturing cognitive rumination and autonomic arousal.\n"
        "   • SleepNet: Temporal Convolutional Network modeling circadian rhythm irregularities and sleep debt.\n"
        "   • FusionNet (Meta-Learner): Non-linear multi-layer perceptron concatenating sub-model embeddings with 23 context features to predict the final 0–100 composite risk score.\n\n"
        "4. Post-Processing, UQ & XAI Layer: Evaluates Monte Carlo Dropout (T = 20) to generate empirical 95% Confidence Intervals and calculates Permutation SHAP values for interactive clinician explanation."
    )
    
    # 4.2 Detailed Design & UML Models
    h4_2 = doc.add_paragraph()
    r4_2 = h4_2.add_run("4.2 Detailed Design & UML Models")
    r4_2.font.size = Pt(13)
    r4_2.font.bold = True
    r4_2.font.color.rgb = BLUE
    h4_2.paragraph_format.space_before = Pt(8)
    h4_2.paragraph_format.space_after = Pt(4)
    
    # 4.2.1 DFD
    h4_2_1 = doc.add_paragraph()
    r4_2_1 = h4_2_1.add_run("4.2.1 Data Flow Diagram (DFD Level 0 & Level 1)")
    r4_2_1.font.size = Pt(11.5)
    r4_2_1.font.bold = True
    r4_2_1.font.color.rgb = NAVY
    
    p_dfd = doc.add_paragraph()
    p_dfd.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
    p_dfd.add_run(
        "• DFD Level 0 (Context Diagram): The adolescent user inputs questionnaire responses, journal text, and lifestyle parameters into the SHRI System. The system processes the inputs locally and returns the Risk Assessment, 95% Confidence Intervals, and CBT Exercises to the Patient, while providing Attending Clinicians with Detailed SHAP Factor Decompositions and Longitudinal Anomaly Flags.\n\n"
        "• DFD Level 1 (Decomposition): Subsystem 1.0 (Data Ingestion & Normalization) → Subsystem 2.0 (NLP Lexical Sentiment Analysis & Crisis Tripwire) → Subsystem 3.0 (Stacked Ensemble Inference across DepNet, AnxNet, SleepNet, and FusionNet) → Subsystem 4.0 (Monte Carlo Uncertainty & SHAP XAI Attribution) → Subsystem 5.0 (Clinical Dashboard & Automated Report Generation)."
    )
    
    # 4.2.2 Use Case Diagram
    h4_2_2 = doc.add_paragraph()
    r4_2_2 = h4_2_2.add_run("4.2.2 Use Case Diagram")
    r4_2_2.font.size = Pt(11.5)
    r4_2_2.font.bold = True
    r4_2_2.font.color.rgb = NAVY
    
    p_uc = doc.add_paragraph()
    p_uc.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
    p_uc.add_run(
        "• Primary Actors: Adolescent Patient, Clinician / School Counselor, System Administrator.\n"
        "• Use Cases for Patient: Complete Psychometric Intake, Submit Journal Entry, View Personalized Coping Strategies, Review Sleep Hygiene Recommendations.\n"
        "• Use Cases for Clinician: Authenticate via Secure Portal, Inspect Composite Risk Index & 95% CI, Analyze Permutation SHAP Waterfall Charts, Review Longitudinal Trajectory Trends, Export FHIR / Clinical PDF Reports.\n"
        "• Use Cases for System Administrator: Monitor Client-Side Tensor Performance, Update Clinical Questionnaires, Calibrate Risk Severity Cutoffs."
    )
    
    # 4.2.3 Class Diagram
    h4_2_3 = doc.add_paragraph()
    r4_2_3 = h4_2_3.add_run("4.2.3 Class Diagram")
    r4_2_3.font.size = Pt(11.5)
    r4_2_3.font.bold = True
    r4_2_3.font.color.rgb = NAVY
    
    p_cd = doc.add_paragraph()
    p_cd.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
    p_cd.add_run(
        "• Core Classes:\n"
        "  - PatientProfile: attributes (patientId, age, gender, assessmentHistory); methods (recordAssessment(), getTrajectory()).\n"
        "  - AssessmentVector: attributes (phq9Scores, gad7Scores, psqiScores, journalSentiment, sleepDuration, screenTime, physicalActivity); methods (normalize(), toTensor()).\n"
        "  - DepNet / AnxNet / SleepNet (Domain Models): attributes (weights, biases, dropoutRate); methods (forward(), computeSubRisk()).\n"
        "  - FusionNet (Meta-Learner): attributes (denseLayers, activationFn); methods (predictComposite(), getEmbeddings()).\n"
        "  - UncertaintyEngine: attributes (mcPasses = 20, dropoutActive = True); methods (calculateConfidenceInterval(), computeEmpiricalStdDev()).\n"
        "  - SHAPExplainer: attributes (baselineVector, numPermutations); methods (computeFeatureAttribution(), generateWaterfallData()).\n"
        "  - ClinicalDashboard: attributes (chartInstances, alertTriggers); methods (renderRadarChart(), displayCBTPlan(), exportPDF())."
    )
    
    # 4.2.4 Sequence Diagram
    h4_2_4 = doc.add_paragraph()
    r4_2_4 = h4_2_4.add_run("4.2.4 Sequence Diagram")
    r4_2_4.font.size = Pt(11.5)
    r4_2_4.font.bold = True
    r4_2_4.font.color.rgb = NAVY
    
    p_sd = doc.add_paragraph()
    p_sd.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
    p_sd.add_run(
        "Execution Sequence: Patient/Clinician enters 23 multimodal parameters → Dashboard triggers Feature Preprocessor → Preprocessor standardizes vector and runs NLP Crisis Scanner → Stacked Ensemble computes sub-scores (DepNet, AnxNet, SleepNet) → FusionNet synthesizes raw risk score → UncertaintyEngine executes 20 stochastic MC Dropout passes → SHAPExplainer calculates marginal contributions for each feature → Dashboard renders interactive charts, severity gauges, 95% CI bands, and CBT recommendations in real time (< 18ms)."
    )
...
if __name__ == "__main__":
    ...
    create_report_docx(out_desktop)
